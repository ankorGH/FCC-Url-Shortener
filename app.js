...
mongoose.connect("mongodb://ankor:dbpass6789@ds259897.mlab.com:59897/urlshortener")
...
app.get("/", function (req, res){
    res.send("Hello got url shortener to build")
 })
 ...
 app.get("/new/*", function(req,res){
     ...
     if(regexUrl.test(userUrl)){
        ...
        urlShortener.find({},function(err,data){
         ...
         let data = urlShortener({id: urlId,url:userUrl,shortenedUrl:shortUrl}).save(function(err, data){
             if(err) throw err;
             console.log("url saved")
         })   
        res.send(JSON.stringify({original_url:userUrl,short_url:req.hostname+"/"+shortUrl}))    
         },
        (err) => {
 ...
 app.get("/:redirectUrl",function(req,res){
     urlShortener.find({shortenedUrl:req.params.redirectUrl},function(err,data){
         if(err) throw err;
